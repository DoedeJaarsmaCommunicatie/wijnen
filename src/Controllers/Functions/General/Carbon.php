...
class Carbon
{
    ...
    public static function is_current_wine_of_the_month($id = null, $type = null): bool
    {
        $current_id = $id ?? get_queried_object_id();

        if ($type && in_array($type, ['white', 'red'])) {
            return $current_id === static::get_wine_of_the_month($type);
        }

        return in_array(
            $current_id,
            [
                static::get_wine_of_the_month('white') ? static::get_wine_of_the_month('white')->get_id() : false,
                static::get_wine_of_the_month('red') ? static::get_wine_of_the_month('red')->get_id() : false,
            ],
            true
        );
    }
